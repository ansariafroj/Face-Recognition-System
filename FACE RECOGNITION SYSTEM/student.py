...
class Student:
    def __init__(self, root):
        ...
        dep_combo["values"]=("Select Department","CSE","CES-CS","Civil","Mechanical","AI&DS","BME")
        dep_combo.current(0)
         ...
         if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                 ...
                 for x in myresult:
                     ...
                     def face_cropped(img,shape):
                         gray=cv2.cvtColor(img,shape,cv2.COLOR_BGR2GRAY)
                         faces=face_cascade.detectMultiScale(gray,1.3,5)
                         for(x,y,w,h) in faces:
                             
                             face_cropped=img.shape[y:y+h,x:x+w]
                             return face_cropped
                     ...
                     while True:
                         ...
                         if cv2.waitKey(1)==13 or int(img_id)==13:
                             break
                     cap.release()
